get-python:
	kubeless function deploy get-python --runtime python2.7 --handler helloget.foo --from-file helloget.py --cpu 0.3 --memory 0.5Gi
...
scheduled-get-python:
	kubeless function deploy scheduled-get-python --schedule "* * * * *" --runtime python2.7 --handler helloget.foo --from-file helloget.py
...
pubsub-nodejs-verify:
	$(eval DATA := $(shell mktemp -u -t XKARTHIKX))
...
python-nats:
	kubeless function deploy python-nats --runtime python2.7 --handler pubsub.handler --from-file nats-python.py
